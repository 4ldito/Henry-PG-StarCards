...
userDecksRoute.get("/:userId", async (req, res, next) => {
  try {
    ...
    if (!user) return res.json({ error: "User Inexist" });
    if (!deckId) {
    ...
    if (!deckFound) return res.json({ error: "Deck Inexist" });
...
userDecksRoute.post("/:userId", async (req, res, next) => {
  ...
  const isValidDeck = newDeckCards.reduce(
    (prev, curr) => (curr.race === newDeckCards[0].race && prev ? true : false),
    true
  );
  ...
  if (!isValidDeck)
    return res.json({ error: "All cards must be the same race" });
  ...
  try {
    ...
    for (const deckCard of newDeckCards) {
      const userCard = await UserCards.findOne({
        where: { CardId: deckCard.id, StatusId: "active", UserId: userId },
      });
...
userDecksRoute.put("/:userId/:id", async (req, res, next) => {
  ...
  try {
    ...
    if (!oldDeck)
      return res.json({ error: "The deck to replace does not exist" });
    ...
    const promisedChanges = cards.map(async (card) => {
      ...
      const userCard = await UserCards.findOne({
        where: { CardId: card.id, StatusId: "active", UserId: userId },
      });
      cardRepeats.push({ userCard: userCard, repeat: card.repeat });
      if (i === 0) {
        i++;
...
userDecksRoute.delete("/:id/:userId", async (req, res, next) => {
  ...
  if (!deckToRemove) return res.json({ error: "Deck Inexist" });
  try {
    await user.removeDeck(deckToRemove);
    await user.save();
    Deck.destroy({ where: { id: parseInt(id) } });
    res.json({ message: "Deck Deleted", deckToRemove });
  } catch (err) {
    next(err);
  }
});

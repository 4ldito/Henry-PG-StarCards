...
function DeckList({
  ...
  function createNewDeck(userId, deck, name) {
    if (name && deck.length && (deck.length > 1 || deck[0].repeat > 1)) {
      const isValidDeck = deck.reduce(
        (prev, curr) => (curr.race === deck[0].race && prev ? true : false),
        true
      );
      ...
      if (!name && !deck.length) {
        setCreationErrors({
          error: "Add some cards to your deck and give it a name",
        });
  ...
  return (
    <div className={bothStacks ? css.deckList : css.deckListOnly}>
      <div
        className={
          bothStacks
            ? css.allContainerWhenBothStacks
            : css.allContainerWhenOnlyDecks
        }
      >
        {/* LISTADO DE MAZOS */}
        ...
        <ul className={css.deckListContainer}>
          {decks?.map((e, i) => (
            <div
              className={
                selectedDeck?.name === e.name
                  ? css.selectedButtonAndDeckNameContainer
                  : css.buttonAndDeckNameContainer
              }
              key={i}
            >
              <div
                className={css.deckName}
                onClick={(e) => findSelectedDeck(e.target.id, decks)}
                id={e.id}
              >
                {e.name}
              </div>
              {decks.length > 1 && (
                <button
                  className={
                    selectedDeck?.name === e.name
                      ? css.deleteButton + " material-symbols-outlined"
                      : css.hidden
                  }
                  key={i}
                  id={e.id}
                  onClick={(e) => {
                    removeDeck(userId, e.target.id);
                  }}
                >
                  delete
                </button>
          ...
          {!creatingDeck && (
            <button
              className={css.addButton + " material-symbols-outlined"}
              onClick={() => openNewDeckTemplate()}
            >
              add
            </button>
          )}
        ...
        {creatingDeck ? (
          <div className={css.inputAndCost}>
            <input
              id="newDeckName"
              autoComplete="off"
          ...
          !selectedDeck?.name && <label>{"Create a deck"}</label>
        )}
        {/* CREACION DE MAZO */}
        <div className={css.actualDeckContainer}>
          <div style={{ color: "black" }} className={css.cardsContainer}>
            {creatingDeck ? (
              <div
                className={bothStacks ? css.creatingDeckContainer : css.only}
              >
              ...
              <div
                className={bothStacks ? css.selectedDeckContainer : css.only}
              >

...
import { getAllUsers, modifyUser, getUser } from "../../redux/actions/user";
...
export default function renderAdmin() {
  ...
  useEffect(() => {
    dispatch(getUser(userActual.id));
  }, []);
  ...
  function listUsers() {
    return (
      <>
        {validToken && userActual.RolId !== "user" ? (
              ...
              <ul>
                {allUsers.map((u) => (
                  <li key={u.id}>
                    ...
                    {
                      <button onClick={(e) => changeRol(e, u)}>
                        {u.RolId === "user" ? "Up to Admin" : "Low to User"}
                      </button>
                    }

                    {
                      <button onClick={(e) => resetPassword(e, u)}>
                        Reset Password
                      </button>
                    }
                  </li>
          ...
          console.log(validToken, userActual.RolId)
        )}
      </>
    );
  }
